...
const pizza_point_kitchen = async (req, res) => {
    ...
    for (let i = 0; i < All.length; i++) {
        const element = All[i];
        if(element.Category == "FOOD Shankha Restaurant"){
    ...
    if (Auths != null) {
        let Login = `
            ...
            `;
        res.status(200).render("Food_shankha_restaurant",{P:fin, NAV: Login, Script1:"HomeLog"});
        ...
        res.clearCookie("ID");
        res.status(200).render("Food_shankha_restaurant",{P:fin, NAV: a, Script1:"Home"});
