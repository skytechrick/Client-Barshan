...
const pizza_point_kitchen = async (req, res) => {
    ...
    for (let i = 0; i < All.length; i++) {
        const element = All[i];
        if(element.Category == "FOOD Kohili Masala Kitchen"){
    ...
    if (Auths != null) {
        let Login = `
            ...
            `;
        res.status(200).render("Food_kohili_masala_restaurant",{P:fin, NAV: Login, Script1:"HomeLog"});
        ...
        res.clearCookie("ID");
        res.status(200).render("Food_kohili_masala_restaurant",{P:fin, NAV: a, Script1:"Home"});

    }
}
module.exports = pizza_point_kitchen;