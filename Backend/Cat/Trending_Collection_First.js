...
const Trending_Collection_First = async (req, res) => {
    ...
    for (let i = 0; i < All.length; i++) {
        const element = All[i];
        if(element.Category == "Trending Collection"){
    ...
    // res.status(200).render("Trending_Collection_First",{P:fin});
    ...
    if (Auths != null) {
        let Login = `
            ...
            `;
        res.status(200).render("Trending_Collection_First",{P:fin, NAV: Login, Script1:"HomeLog"});
        ...
        res.clearCookie("ID");
        res.status(200).render("Trending_Collection_First",{P:fin, NAV: a, Script1:"Home"});

    }
}
module.exports = Trending_Collection_First;