...
const pizza_point_kitchen = async (req, res) => {
    ...
    for (let i = 0; i < All.length; i++) {
        const element = All[i];
        if(element.Category == "Grocery Tooth Brush & Tooth Paste"){
    ...
    if (Auths != null) {
        let Login = `
            ...
            `;
        res.status(200).render("Grocery_tooth_brush_paste",{P:fin, NAV: Login, Script1:"HomeLog"});
        ...
        res.clearCookie("ID");
        res.status(200).render("Grocery_tooth_brush_paste",{P:fin, NAV: a, Script1:"Home"});
