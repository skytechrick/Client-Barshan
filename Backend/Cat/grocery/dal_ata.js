...
const pizza_point_kitchen = async (req, res) => {
    ...
    for (let i = 0; i < All.length; i++) {
        const element = All[i];
        if(element.Category == "Grocery Dal & Ata"){
    ...
    if (Auths != null) {
        let Login = `
            ...
            `;
        res.status(200).render("Grocery_dal_ata",{P:fin, NAV: Login, Script1:"HomeLog"});
        ...
        res.clearCookie("ID");
        res.status(200).render("Grocery_dal_ata",{P:fin, NAV: a, Script1:"Home"});
