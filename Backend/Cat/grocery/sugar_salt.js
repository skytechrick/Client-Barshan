...
const pizza_point_kitchen = async (req, res) => {
    ...
    for (let i = 0; i < All.length; i++) {
        const element = All[i];
        if(element.Category == "Grocery Sugar & Salt"){
    ...
    if (Auths != null) {
        let Login = `
            ...
            `;
        res.status(200).render("Grocery_sugar_salt",{P:fin, NAV: Login, Script1:"HomeLog"});
        ...
        res.clearCookie("ID");
        res.status(200).render("Grocery_sugar_salt",{P:fin, NAV: a, Script1:"Home"});
